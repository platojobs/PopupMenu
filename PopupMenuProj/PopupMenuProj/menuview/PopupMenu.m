...
#pragma clang diagnostic ignored "-Wundeclared-selector"
        if ([v isKindOfClass:[PopupMenu class]] && [v respondsToSelector:@selector(dismissMenu:)]) {
...
+ (instancetype) menuItem:(NSString *) title
                   ...
                   action:(SEL) action
{
    return [[PopupMenuItem alloc] init:title
                              image:image
                             target:target
                             action:action];
}
           ...
           withOptions:(OptionalConfiguration) options
{
    ...
    PopupMenuOverlay *overlay = [[PopupMenuOverlay alloc] initWithFrame:view.bounds maskSetting:self.kxMenuViewOptions.maskToBackground];
...
- (UIView *) mkContentView
{
    ...
    UIFont *titleFont = [Popup titleFont];
    ...
    UIImage *selectedImage = [PopupMenu selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    //配置：分隔线是与内容等宽还是与菜单等宽
    int insets = 0;
    ...
    UIImage *gradientLine = [PopupMenu gradientLine: (CGSize){maxItemWidth- kMarginX * insets, 0.4}];
             ...
             inContext:(CGContextRef) context
{
    ...
    UIColor *tintColor = [Popup tintColor];
    if (tintColor) {
            ...
            withOptions:(OptionalConfiguration) options
{
    ...
    _menuView = [[PopupMenu alloc] init];
